...
public class Utils
{
    ...
    public static String parseChatUrl(String stringToParse, User sender)
    {
        try
        {
            ...
            if (stringToParse.matches("^https?://(www\\.)?youtube\\.com/watch.*"))
            {
                ...
                return colourise(String.format("(%s's URL) &2Title: &r%s &2Uploader: &r%s &2Views: &r%s &2Rating: &a%s&r/&c%s", munge(sender.getNick()), StringEscapeUtils.unescapeHtml4(title), uploader, views, likes,
                                               dislikes));
            }
            ...
            if (stringToParse.matches("^https?://(www\\.)?reddit\\.com/r/.*/comments/.*"))
            {
                ...
                return colourise(String.format("(%s's URL) &2Title: &r%s &2Poster: &r%s &2Comments: &r%s &2Rating: &6%s&r/&9%s", munge(sender.getNick()), StringEscapeUtils.unescapeHtml4(title), poster, comments,
                                               likes, dislikes));
            }
    ...
    {
        return foxbot.getConfig().getMungeUsernames() ? stringToMunge.replace("a", "ä").replace("e", "è").replace("o", "ö").replace("u", "ù").replace("s", "š").replace("i", "í").replace("n",
                "ñ") : stringToMunge;
    }
