...
public class Utils
{
    ...
    public static String parseChatUrl(String stringToParse, User sender)
    {
        try
        {
            ...
            {
                return colourise(String.format("(%s's URL) &cError: &r%s %s ",
                        munge(sender.getNick()), response.statusCode(), response.statusMessage()));
            }
            ...
            if (stringToParse.matches("^https?://(www\\.)?youtube\\.com/watch.*"))
            {
                ...
                return colourise(String.format("(%s's URL) &2Title: &r%s &2Uploader: &r%s &2Views: &r%s &2Rating: &a%s&r/&c%s",
                        munge(sender.getNick()), StringEscapeUtils.unescapeHtml4(title), uploader, views, likes, dislikes));
            }
